...
interface Sample {
  t: number
  bytes: number
  /** bytes queued on the data channel — high and flat means *we* are the limit */
  buffered: number
  rttMs?: number
  /** what the browser thinks the path can carry */
  outKbps?: number
}
...
export class TransferDiag {
        ...
        stats.forEach((r: any) => {
          if (r.type === 'candidate-pair' && (r.nominated || r.state === 'succeeded')) {
            ...
            if (typeof r.availableOutgoingBitrate === 'number') {
              sample.outKbps = Math.round(r.availableOutgoingBitrate / 1000)
            }
  ...
  report(outcome: string, totalBytes: number): string {
    ...
    if (this.samples.length > 1) {
      lines.push('t(s)   MB     rate      buffered   rtt     est.link')
      ...
      for (const s of this.samples.slice(1)) {
        ...
        lines.push(
          [
            ...
            `${(s.buffered / 1024).toFixed(0)} KB`.padStart(10),
            s.rttMs !== undefined ? `${s.rttMs.toFixed(0)}ms`.padStart(7) : '      -',
            s.outKbps !== undefined ? `${(s.outKbps / 1024).toFixed(1)} MB/s`.padStart(9) : '        -',
          ].join(' ')
        )
        prev = s
      }
